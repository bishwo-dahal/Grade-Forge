...
**Goal:** Build a functional monthly calendar page for students and faculty that shows assignment due dates as colored dots, lets users click a date to see that day's assignments in a right panel, and lists "Upcoming This Week" cards below the grid.

**Architecture:** A single `CalendarPage.tsx` component accepts a `roleView` prop and fetches data from the matching snapshot service. Pure helper functions handle grid generation, date keying, and week-range logic. The two placeholder exports in `RoleWorkspacePages.tsx` are replaced to delegate to `CalendarPage`.

**Tech Stack:** React 18, TypeScript, Tailwind CSS 4, lucide-react, react-router `useNavigate`, `studentCourseworkService`, `facultyCourseworkService`
...
| Action | Path | Purpose |
|--------|------|---------|
| Modify | `Client/src/types/assignment.ts` | Add `CalendarAssignment` type |
| Create | `Client/src/app/components/CalendarPage.tsx` | Full calendar UI + data logic |
| Modify | `Client/src/app/components/RoleWorkspacePages.tsx` | Replace two placeholder exports |

---

### Task 1: Add CalendarAssignment type

**Files:**
- Modify: `Client/src/types/assignment.ts`

- [ ] **Step 1: Append the type after `StudentAssignmentListItem` (after line 90)**

Add to `Client/src/types/assignment.ts`:
...
```bash
cd Grade-Forge
git add Client/src/types/assignment.ts
git commit -m "feat(calendar): add CalendarAssignment type"
```

---

### Task 2: Create CalendarPage.tsx

**Files:**
- Create: `Client/src/app/components/CalendarPage.tsx`

- [ ] **Step 1: Create the file with all helpers, data loaders, and the full component**
...
```bash
cd Grade-Forge
git add Client/src/app/components/CalendarPage.tsx
git commit -m "feat(calendar): create CalendarPage with grid, day detail, and upcoming section"
```

---

### Task 3: Wire CalendarPage into RoleWorkspacePages

**Files:**
- Modify: `Client/src/app/components/RoleWorkspacePages.tsx`

- [ ] **Step 1: Add import at the top of the file (after the existing imports)**

In `Client/src/app/components/RoleWorkspacePages.tsx`, add after the last import line:

```typescript
import { CalendarPage } from "./CalendarPage";
```

- [ ] **Step 2: Replace the StudentCalendarPage export (line 80–82)**
...
- [ ] **Step 3: Replace the FacultySchedulePage export (line 106–108)**
...
```bash
cd Grade-Forge/Client
npm run build 2>&1 | tail -30
```

Expected: exits 0. If you see a type error on `roleView`, check that `AuthShell` accepts `"student" | "faculty"` — it does per its `AuthShellProps` interface.

- [ ] **Step 5: Commit**

```bash
cd Grade-Forge
git add Client/src/app/components/RoleWorkspacePages.tsx
git commit -m "feat(calendar): wire CalendarPage into student and faculty routes"
```
...
1. Start dev server: `cd Grade-Forge/Client && npm run dev`
2. Sign in as a student → navigate to **Calendar** in sidebar
   - Calendar grid renders with current month
   - Today's cell has an orange border
   - Dates with assignments show colored dots
   - Clicking a date with assignments → right panel shows those assignments with course badge, time, and points
   - Clicking a date with no assignments → right panel shows "No assignments due"
   - Prev/Next month buttons change the displayed month
   - "Upcoming This Week" cards appear below the grid (if any assignments fall in the current week)
   - Clicking an assignment card in either the right panel or the weekly section navigates to `/assignment/:id`
3. Sign in as faculty → navigate to **Calendar** in sidebar
   - Same grid behavior but data comes from faculty snapshot
   - Clicking an assignment card navigates to `/faculty/class/:courseId/assignment/:id`
4. Resize browser to mobile width — calendar and day detail panel stack vertically via flex-wrap behavior
