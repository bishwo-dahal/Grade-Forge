# Sign-in and account

## How do I create an account?

1. Open **Sign up** from the public landing page.
2. Complete the form. Your school may restrict who may register.

**Path:** `/signup`  
**Example URL:** `https://gradeforge.tech/signup`

![Sign-up page on the public site](/docs/manual/images/signup.png)

## How do I sign in?

1. Open **Sign in**.
2. Enter credentials your institution uses.

**Path:** `/signin`  
**Example URL:** `https://gradeforge.tech/signin`

![Sign-in page on the public site](/docs/manual/images/signin.png)

## How do I finish student registration the first time?

1. After login, if the app sends you there, open **Complete registration**.
2. Submit required fields.

**Path:** `/complete-registration`  
**Example URL:** `https://gradeforge.tech/complete-registration`

## How do I change my profile, password, notifications, or theme?

**Students, faculty, and grading assistants**

1. Open **Settings** in the sidebar.
2. Choose **Profile**, **Security**, **Notifications**, or **Appearance** and save.

**Path:** `/settings`  
**Example URL:** `https://gradeforge.tech/settings`

**University administrators**

1. Use **Settings** inside the university workspace (not the same route as students).

**Path:** `/university-admin/settings`  
**Example URL:** `https://gradeforge.tech/university-admin/settings`

## Common issues

- **Forgot password:** use your school’s reset flow if the sign-in page offers it.
- **Wrong workspace after login:** your **role** controls which shell you see. Contact your admin if the role is wrong.
